...
import { deleteHealthPlanById } from 'apiSdk/health-plans';
...
function OrganizationViewPage() {
  ...
  const health_planHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('health_plan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/health-plans/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.health_plan?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/health-plans/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            ...
                            <Td>
                              {hasAccess('health_plan', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/health-plans/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('health_plan', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    health_planHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
